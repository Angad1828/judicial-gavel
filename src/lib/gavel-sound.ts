 ...
 *
 * Ported from the LegalLens startup prototype so the sound behaves
 * exactly as in that demo.
 */
...
function measureKnocks(buffer: AudioBuffer): number[] {
  ...
  for (let i = 0; i + win < ch.length; i += win) {
    let s = 0;
    for (let j = i; j < i + win; j++) s += (ch[j] ?? 0) * (ch[j] ?? 0);
  ...
  for (let i = 0; i < rms.length; i++) {
    ...
    if (!inSpike && r > peak * 0.25) {
      inSpike = true;
      starts.push(t);
    } else if (inSpike && r < peak * 0.1) {
      inSpike = false;
    }
  }
  ...
  if (knocks.length === 0) {
    ...
    if (first >= 0) knocks.push(((first ?? 0) * win) / sr);
  }
  return knocks;
}
